...
5. Try both practice topics, reveal feedback, visit a source passage and return to the unfinished answer. Bookmark a passage, download its source notes, and delete the lecture. An unavailable companion should retain the same reopening link rather than redirect elsewhere.

The ordinary demo stores sessions only in the local service's memory; they expire after 30 minutes or server restart. Bookmarks/answers are page-local and clear on navigation/deletion. A notes download is a separate file the user keeps/deletes. The setup has no shared class, account or production privacy guarantee. It is intended for synthetic demonstration data.
