...
| TASK-103 Actual AI help evaluation           |    1 | IN PROGRESS | Codex Coordinator | Product Owner Execute next; Phase A contract approved; actual-model evidence needs separate capped authorization | Bounded preparation only; `docs/tasks/TASK-103.md`; `coord/TASK-103-ai-readiness`; exclusive uploader/backend/shared/evaluation lanes                      |
...
Next concrete work: execute TASK-103 Phase A's approved bounded preparation, then arrange the separate real-model trial and TASK-303 human checks. Human Chrome inspection still requires session-specific permission. Judge access and submission scope remain unresolved. Keep the current M2 service available; the new rehearsal route is present on main but is not served by the preserved older server. Live audio remains conditional and M4 breadth stays deferred until its gates pass.
...
- The accompanying coordination revision records this transaction. Phase A becomes MERGED only after exact final-head independent approval, full clean-install CI including production HTTP/build and history secret scanning, and independent promotion of the approved tree to main. Until then Phase A is IN REVIEW. TASK-103 remains IN PROGRESS because actual-model evaluation stays open; no milestone acceptance is implied. The integration handoff records the reviewed head, resulting main and CI. See `docs/evaluations/TASK-103/README.md` for frozen cases and remaining trial requirements.
