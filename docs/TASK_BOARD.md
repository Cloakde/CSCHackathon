...
| TASK-103C Gemini assistance migration        |    1 | TODO        | Claude            | User provider choice; shared-branch handoff; Gemini or Codex independent review                              | `docs/tasks/TASK-103C.md`; text assistance only, zero provider traffic during preparation                                                                  |
...
Next concrete product work: Claude implements TASK-103C to replace the inactive OpenAI trial with Gemini. Gemini then independently reviews that migration and closes the remaining M3 evidence before optional M4 work. [AI_ASSIGNMENTS.md](AI_ASSIGNMENTS.md) gives the sequence and stop conditions. The prepared OpenAI implementation is historical at `8cfa83b88c0f6186d3475266b005069da4fbe820` ([PR 4](https://github.com/Cloakde/CSCHackathon/pull/4)); do not run its provider proposal. The Gemini model/configuration needs official-source verification, and any real trial still needs independent review, required CI, explicit capped authorization and local credentials. Human Chrome/learner, judge access and calendar decisions remain open.
...
| Actual AI evaluation owner/reviewer and frozen cases                  | **ASSIGNED — Claude migrates under TASK-103C; Gemini or Codex reviews; four frozen cases retained**                                         |
| Actual AI response-time targets and capped provider-run authorization | **10s Help / 4s practice; Gemini model/accounting pending Claude verification; proposal at most $1 / 32 attempts; $0 authorized**           |
