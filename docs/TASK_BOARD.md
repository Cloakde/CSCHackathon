...
The Product Owner requested one shared branch with AIs taking turns on 2026-09-06. All current work uses **`shared/livelecture`** in **`C:\Users\abuiz\Documents\Codex\2026-09-04\CSCHackathon`**, created from verified clean main `8cfa83b88c0f6186d3475266b005069da4fbe820`. One AI works at a time, including review. Do not start parallel AI workers or create new task/model worktrees. The current AI leaves a short handoff before the next AI starts. [ADR 0011](adr/0011-sequential-shared-branch.md) is the authoritative decision.

Normal checkpoint pushes to the shared branch are allowed; no force-push or history rewrite. `main` remains the reviewed baseline and promotion still requires applicable review/checks by an AI that did not author the implementation. Keep the persistent shared branch after promotion. Historical task branches and role assignments below record previous work, not instructions to resume those checkouts. Existing worktrees, dirty files, provider/spending gates and original MeltingPot boundaries remain protected.

The shared-branch workflow checkpoint `48172a2b82e9615b4fc1b39217b37868ae006d09` remains IN REVIEW for promotion to main. The user now assigns Claude the Gemini migration, then Gemini the eligible milestones, with Codex providing senior direction. Current Codex work is coordination documentation only: AGENTS/README, handoff, AI assignments, workflow, this board, TASK-103B/103C, ADR 0010/0012 and TASK-103 evaluation documentation. The implementation is left for Claude; no new milestone acceptance is claimed.
...
| TASK-103 Actual AI help evaluation           |    1 | IN PROGRESS | Codex Coordinator | Phase A merged; Phase B contract approved; real-model evidence needs separate capped authorization           | Historical trial preparation merged; Gemini migration and current run gate in `docs/tasks/TASK-103C.md`; shared branch                                     |
| TASK-103C Gemini assistance migration        |    1 | IN REVIEW   | Claude            | User provider choice; shared-branch handoff; Gemini or Codex independent review                              | Implementation record and exact source hash in `docs/tasks/TASK-103C.md`; text assistance only, zero provider traffic; awaiting independent review         |
...
Next concrete product work: Claude's TASK-103C migration is implemented and IN REVIEW (see the implementation record in `docs/tasks/TASK-103C.md` for the exact source hash, verified Gemini facts, changed files and commands/results). Gemini or Codex now independently reviews that migration, then Gemini closes the remaining M3 evidence before optional M4 work. [AI_ASSIGNMENTS.md](AI_ASSIGNMENTS.md) gives the sequence and stop conditions. The prepared OpenAI implementation is historical at `8cfa83b88c0f6186d3475266b005069da4fbe820` ([PR 4](https://github.com/Cloakde/CSCHackathon/pull/4)); do not run its provider proposal. Any real Gemini trial still needs independent review, required CI, explicit capped authorization and a local `GEMINI_API_KEY`. Human Chrome/learner, judge access and calendar decisions remain open.
...
| Available engineering lanes              | **ONE ACTIVE AI — Claude implements TASK-103C, then Gemini reviews/continues; Codex provides senior scope/review; docs/HANDOFF.md controls the turn**  |
...
| Actual AI evaluation owner/reviewer and frozen cases                  | **IN REVIEW — Claude's TASK-103C migration implemented; Gemini or Codex reviews next; four frozen cases retained**                                          |
| Actual AI response-time targets and capped provider-run authorization | **10s Help / 4s practice; `gemini-2.5-flash-lite` verified (ADR 0012); reservation 105,677 µUSD/attempt; proposal at most $1 / 32 attempts; $0 authorized** |
