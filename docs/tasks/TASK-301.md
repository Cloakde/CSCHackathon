...
- Relay/runtime-schema checks reject wrong session, concept, event, evidence, active/missing/deleted/expired data, malformed or oversized responses, bad Host/Origin/header/path/method/body, redirects, and timeout. Only the fixed upstream is called; no credentials are forwarded.
