...
Reject queries, unknown/encoded operation paths, unsupported methods, invalid canonical IDs, bodies on GET/DELETE, and POST bodies over 1 KiB. Require JSON for POST. Bound each upstream operation, including body reading, to five seconds and each response to 256 KiB. Abort upstream work on cancellation; do not follow redirects. Bound request rates to 240 per minute per process. Failures return fixed safe messages and existing API error envelopes, not raw upstream errors.
