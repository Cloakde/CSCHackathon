...
This decision supersedes old requirements for a separate branch/worktree per task or model and for parallel engineering lanes, including in historical contracts. Preserve old branches and worktrees without treating them as current starting points or merging historical model branches wholesale. Preserve dirty files, check the actual current state, and avoid repeating completed work just because the active model changed.
