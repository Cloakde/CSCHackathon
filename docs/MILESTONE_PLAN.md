...
**Current work order (2026-09-07):** The user assigned Codex to fix the five reviewed defects in [TASK-306](tasks/TASK-306.md), which connects Help, targeted practice, Ask and Catch Me Up to Gemini using offline fake transports. Normal launches still use prewritten help; preparing guarded provider code is not approval to execute it. TASK-304/305 and the new correction need independent review. Actual-model evidence, human acceptance, broader M4 work and M5/M6 gates remain pending. Do not request keys/spend or silently resume provider tests. The generated-practice handoff currently uses the LiveLecture companion because the isolated MeltingPot copy still declares assistance prewritten; updating that copy remains a separate dependency.
